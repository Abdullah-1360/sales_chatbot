...
// CRITICAL SAFETY RULES - BACKUP FIRST APPROACH:
// 1. ALWAYS create timestamped backup before any destructive operations
// 2. For corrupted plugins/themes: FIRST backup, THEN deactivate, THEN remove, THEN reinstall
// 3. Use "cp -r wp-content/plugins/[plugin-name] wp-content/plugins/[plugin-name].backup.$(date +%Y%m%d_%H%M%S)" for plugin backup
// 4. Use "cp -r wp-content/themes/[theme-name] wp-content/themes/[theme-name].backup.$(date +%Y%m%d_%H%M%S)" for theme backup
// 5. After backup, use "wp plugin deactivate [plugin-name] --skip-plugins --allow-root" to safely deactivate
// 6. After deactivation, use "wp plugin delete [plugin-name] --allow-root" to remove
// 7. Then use "wp plugin install [plugin-name] --force --allow-root" to reinstall fresh copy
// 8. NEVER use "rm -rf" commands without backup first
// 9. Focus on complete file replacement rather than editing
...
// Step 2: Deactivate plugin/theme using WP-CLI (if possible)
...
// 1. FIRST: Backup the corrupted plugin/theme directory
// 2. SECOND: Try WP-CLI deactivate (may fail due to parse errors)
// 3. THIRD: Remove corrupted directory (only after backup exists)
// 4. FOURTH: Reinstall fresh files using wp-cli or direct download
// 5. FIFTH: Verify the fix worked
...
// - ALWAYS backup corrupted files before fixing (MANDATORY)
// - Deactivate plugins/themes safely when possible
// - Remove corrupted plugin/theme directories only after backup
...
// PRODUCTION SAFETY REQUIREMENTS - BACKUP FIRST MANDATORY:
...
// BACKUP COMMANDS (MANDATORY FORMAT):
// - Plugin: "cp -r wp-content/plugins/[name] wp-content/plugins/[name].backup.$(date +%Y%m%d_%H%M%S)"
// - Theme: "cp -r wp-content/themes/[name] wp-content/themes/[name].backup.$(date +%Y%m%d_%H%M%S)"
// - Verify: "ls -la wp-content/plugins/[name].backup.* || ls -la wp-content/themes/[name].backup.*"

// DEACTIVATION COMMANDS (MANDATORY FORMAT):
// - Plugin: "wp plugin deactivate [plugin-name] --skip-plugins --allow-root"
// - Theme: "wp theme activate twentytwentyfour --skip-themes --allow-root" (switch to default theme)
// - The --skip-plugins flag is CRITICAL to bypass plugin conflicts during deactivation
// - The --skip-themes flag is CRITICAL to bypass theme conflicts during activation
// - Always use --skip-plugins and --skip-themes to ensure operations work even with corrupted files

// PRODUCTION REPAIR WORKFLOW:
// 1. BACKUP: Always create timestamped backup first
// 2. VERIFY: Confirm backup exists before proceeding
// 3. DEACTIVATE: Use WP-CLI with --skip-plugins flag (wp plugin deactivate [name] --skip-plugins --allow-root)
//    For themes: Use --skip-themes flag (wp theme activate twentytwentyfour --skip-themes --allow-root)
// 4. REMOVE: Use WP-CLI to delete corrupted files (wp plugin delete [name] --allow-root)
// 5. REINSTALL: Use WP-CLI to install fresh copy (wp plugin install [name] --force --allow-root)
// 6. VERIFY: Confirm successful installation
// 7. REACTIVATE: Activate if needed and verify functionality

// THEME REPAIR SPECIAL WORKFLOW:
// 1. BACKUP: Create timestamped backup of corrupted theme
// 2. INSTALL FALLBACK: Install a default theme with multiple fallbacks
//    Command: wp theme install twentytwentyfour --force --allow-root || wp theme install twentytwentythree --force --allow-root || wp theme install hello-elementor --force --allow-root
// 3. ACTIVATE FALLBACK: Switch to default theme (wp theme activate twentytwentyfour --skip-themes --allow-root || wp theme activate twentytwentythree --skip-themes --allow-root || wp theme activate hello-elementor --skip-themes --allow-root)
// 4. REMOVE CORRUPTED: Delete the corrupted theme (wp theme delete [corrupted-theme] --allow-root)
// 5. REINSTALL: Install fresh copy with fallbacks
//    Command: wp theme install [theme-name] --force --allow-root || wget https://downloads.wordpress.org/theme/[theme-name].zip && unzip [theme-name].zip -d wp-content/themes/ && rm [theme-name].zip
// 6. REACTIVATE: Switch back to repaired theme (wp theme activate [theme-name] --allow-root)
// 7. CLEANUP: Optionally remove fallback theme if not needed

// FALLBACK THEME STRATEGY:
// - Always install a working theme before removing corrupted themes
// - Use multiple installation methods with || operator for reliability
// - Primary: wp theme install twentytwentyfour --force --allow-root
// - With fallbacks: wp theme install twentytwentyfour --force --allow-root || wp theme install twentytwentythree --force --allow-root || wp theme install twentytwentytwo --force --allow-root
// - If all default themes fail, try: wp theme install storefront --force --allow-root || wp theme install hello-elementor --force --allow-root || wp theme install astra --force --allow-root
// - Never leave a site without an active working theme
// - Install fallback theme with --force flag to ensure availability

// MULTI-COMMAND FALLBACK FORMAT:
// - Use || operator to chain multiple fallback commands
// - Format: "primary_command || fallback_command_1 || fallback_command_2"
// - Each command will only run if the previous one fails
// - Ensures maximum reliability for critical operations
// - Example: wp theme install twentytwentyfour --force --allow-root || wp theme install twentytwentythree --force --allow-root || wp theme install hello-elementor --force --allow-root

// TIMEOUT HANDLING FOR DOWNLOADS:
// - Plugin/theme downloads may take 60-120 seconds
// - Core downloads may take 120-300 seconds
// - Always use --force flag for reinstallation
// - If download fails, retry once with increased timeout
// - Use multi-command format with || operator for reliability
// - Plugin install example: wp plugin install [plugin-name] --force --allow-root || wget https://downloads.wordpress.org/plugin/[plugin-name].zip && unzip [plugin-name].zip -d wp-content/plugins/ && rm [plugin-name].zip
// - Theme install example: wp theme install [theme-name] --force --allow-root || wget https://downloads.wordpress.org/theme/[theme-name].zip && unzip [theme-name].zip -d wp-content/themes/ && rm [theme-name].zip
...
// - Always have rollback plan using backups
// - Use --skip-plugins flag to bypass plugin conflicts during deactivation

// CRITICAL DEACTIVATION RULES:
// - ALWAYS use --skip-plugins flag when deactivating plugins
// - ALWAYS use --skip-themes flag when activating themes
// - This prevents plugin/theme conflicts from blocking operations
// - Plugin format: "wp plugin deactivate [plugin-name] --skip-plugins --allow-root"
// - Theme format: "wp theme activate twentytwentyfour --skip-themes --allow-root"
// - These flags bypass conflicts and ensure reliable operations

// CRITICAL THEME REPAIR RULES:
// - NEVER remove a theme without installing a fallback theme first
// - ALWAYS install a working theme before deleting corrupted themes
// - Use multi-command format with || operator for maximum reliability
// - Primary fallback: wp theme install twentytwentyfour --force --allow-root || wp theme install twentytwentythree --force --allow-root || wp theme install hello-elementor --force --allow-root
// - If primary fallbacks fail, try: wp theme install storefront --force --allow-root || wp theme install astra --force --allow-root
// - Ensure website remains functional throughout the repair process
// - Test fallback theme activation with multiple options
// - Use || operator to chain installation and activation commands
...
// - Include proper timeout values for download operations
// - Provide clear reasoning for each action
// - Focus on reliability and safety over speed
// - Always include fallback strategies
// - ALWAYS use --skip-plugins flag for plugin deactivation
// - ALWAYS use --skip-themes flag for theme activation
// - ALWAYS install fallback theme before removing corrupted themes
// - NEVER leave a website without a working theme
// - USE || operator for multi-command fallbacks when installing themes/plugins
// - GENERATE 2-3 fallback commands chained with || for critical operations
...
// 6. After backup, use "wp plugin deactivate [name] --skip-plugins --allow-root" to safely deactivate
// 7. After deactivation, use "wp plugin delete [name] --allow-root" to remove
...
// FALLBACK STRATEGIES (BACKUP-FIRST):
// - If "wp plugin deactivate" fails, first backup then use direct file operations
// - If "wp plugin delete" fails, first backup then use "rm -rf wp-content/plugins/[plugin-name]"
